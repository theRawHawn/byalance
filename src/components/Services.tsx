...
import { Calculator, FileText, Landmark, Users, Receipt, ShieldCheck } from 'lucide-react';
...
export default function Services() {
  ...
  const serviceIcons = [Calculator, FileText, Landmark, Receipt, Users, ShieldCheck];
  const serviceLinks = [
  ...
  const serviceStyles = [
    ...
    {
      cardHover: "hover:border-emerald-200/80 hover:shadow-2xl hover:shadow-emerald-100/55",
      iconBg: "bg-emerald-50/80 text-emerald-600 border-emerald-100",
      iconGroupHover: "group-hover:bg-emerald-600 group-hover:text-white group-hover:border-emerald-600",
      btnLearn: "border-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white hover:border-emerald-600",
      btnMobile: "bg-emerald-50 text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white",
      bulletBg: "bg-emerald-50 text-emerald-600"
    }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 gap-8 max-w-6xl mx-auto">
          {t.services.items.map((service: any, index: number) => {
            ...
            const elementId = serviceLinks[index] ? serviceLinks[index].split('/').pop() : `service-${index}`;
            return (
              <motion.div
                key={index}
                id={elementId}
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-16 text-center max-w-2xl mx-auto border-t border-slate-100 pt-10"
          >
            <h4 className="text-base md:text-lg font-display font-bold text-slate-900 mb-2">{t.services.ctaTitle}</h4>
            <p className="text-slate-500 text-sm leading-relaxed mb-6">{t.services.ctaText}</p>
            <a 
              href="https://wa.me/917406296116"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center px-6 py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs tracking-wide transition-all shadow-md shadow-indigo-100 hover:shadow-lg hover:shadow-indigo-200/50 gap-2 group transform hover:-translate-y-0.5 active:translate-y-0"
            >
              {t.services.ctaButton} <span className="text-xs transition-transform duration-300 group-hover:translate-x-0.5">→</span>
            </a>
          </motion.div>
        </div>
      </div>
